...
class ConfigHotLoader:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        self.new_credentials_path = self.new_config_dir / "credentials.json"
    ...
    def _load_config(self):
        ...
        # 1. 从 credentials.json 读取敏感配置（优先）
        credentials_configs = self._load_from_credentials()
        if credentials_configs:
            configs.update(credentials_configs)

        # 2. 从 .env 读取静态配置（降级）
        if not credentials_configs:
            env_configs = self._load_from_env()
            if env_configs:
                configs.update(env_configs)
                self.logger.warning("使用 .env 作为降级配置")

        # 3. 从新的 admin-backend/config/ 读取配置
    ...
    def _load_from_credentials(self) -> Dict[str, Any]:
        """
        函数级注释：从 credentials.json 读取敏感配置
        """
        configs = {}

        if not self.new_credentials_path.exists():
            return configs

        try:
            with open(self.new_credentials_path, 'r', encoding='utf-8') as f:
                data = json.load(f)

            # 飞书配置
            feishu = data.get("feishu", {})
            if feishu.get("app_id"):
                configs["feishu_app_id"] = feishu["app_id"]
            if feishu.get("app_secret"):
                configs["feishu_app_secret"] = feishu["app_secret"]
            if feishu.get("group_chat_id"):
                configs["feishu_group_chat_id"] = feishu["group_chat_id"]

            # 阿里云配置
            aliyun = data.get("aliyun", {})
            if aliyun.get("access_key_id"):
                configs["ali_access_key_id"] = aliyun["access_key_id"]
            if aliyun.get("access_key_secret"):
                configs["ali_access_key_secret"] = aliyun["access_key_secret"]
            if aliyun.get("sms_sign_name"):
                configs["ali_sms_sign_name"] = aliyun["sms_sign_name"]
            if aliyun.get("sms_template_code"):
                configs["ali_sms_template_code"] = aliyun["sms_template_code"]

            # 记录 mtime
            self._config_mtime["credentials"] = self.new_credentials_path.stat().st_mtime
            self.logger.info(f"加载 credentials.json 成功")
        except Exception as e:
            self.logger.error(f"加载 credentials.json 失败: {e}")
    ...
    def _check_changes(self):
        ...
        if self.new_credentials_path.exists():
            mtime = self.new_credentials_path.stat().st_mtime
            if mtime > self._config_mtime.get("credentials", 0):
                self.logger.info("检测到 credentials.json 变更，重新加载...")
